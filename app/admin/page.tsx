...
export default function AdminPage() {
  ...
  const handleExitAdmin = useCallback(() => {
    ...
    router.push('/');
  }, [isEditMode, toggleEditMode, router]);
  ...
  useEffect(() => {
    ...
    if (wasEditMode && !isEditMode) {
      const currentConfigStr = JSON.stringify(pageConfigRef.current);
    ...
    const currentConfigStr = JSON.stringify(pageConfig);
    if (currentConfigStr === lastSavedConfigRef.current) {
      return;
    }
    ...
    saveTimerRef.current = setTimeout(() => {
      const configToSave = pageConfigRef.current;
  ...
  return (
    <main className="relative min-h-screen w-full overflow-hidden">
      ...
      <button
        ...
        title="返回主页"
        className={`${FLOATING_BUTTON_CLASS} ${FLOATING_BUTTON_IDLE_CLASS} left-6`}
      >
        <svg viewBox="0 0 20 20" className="h-5 w-5 fill-white/70" aria-hidden="true">
          <path
            fillRule="evenodd"
